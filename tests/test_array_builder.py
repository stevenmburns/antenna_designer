...
def test_bowtiearray2x4_optimize():
    ...
    builder = ant.optimize(
        bowtiearray2x4.Builder(gold_params),
        [
            "length_otop",
            "slope_otop",
            "length_obot",
...
def test_invveearray_optimize():
    ...
    params = ant.optimize(
        b,
        ["length_top", "slope_top", "length_bot", "slope_bot"],
        z0=50,
        resonance=True,
        opt_gain=True,
    )
...
def test_bowtiearray_phase_lr_pattern():
    ...
    builders = (
        bowtiearray.Builder(dict(bowtiearray.Builder.default_params, **{"phase_lr": p}))
        for p in np.linspace(0, 180 - 36, 5)
    )
...
def test_bowtiearray_phase_tb_pattern():
    ...
    builders = (
        bowtiearray.Builder(dict(bowtiearray.Builder.default_params, **{"phase_tb": p}))
        for p in np.linspace(-60, 60, 5)
    )
...
def test_bowtiearray_optimize():
    ...
    builder = ant.optimize(
        bowtiearray.Builder(gold_params),
        ["length_top", "slope_top", "length_bot", "slope_bot"],
        z0=200,
    )
    params = builder._params
...
def test_bowtiearray_optimize_for_gain():
    ...
    builder = ant.optimize(
        bowtiearray.Builder(gold_params),
        ["length_top", "slope_top", "length_bot", "slope_bot", "del_y", "del_z"],
        z0=200,
        resonance=True,
        opt_gain=True,
    )
